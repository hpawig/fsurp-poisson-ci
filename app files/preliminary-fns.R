...
find_roots <- function(a, b, conf.level, root) {
  if (root %in% c(1,2) == FALSE) {stop("please specify root = 1 or root = 2.")}
  # getting root 1
  ac.max <- AC_max_coords(a,b)$lambda
  end <- ac.max # end root1 search where current AC's max is
  ...
  if (root == 1 & a != 0) {
    ...
    # Does not exist
    stop("root1 does not exist for x = 0")
  } else {
    ## now finding root 2
    
    # this loop finds the interval of where to search for root2
    start <- ac.max  # start at current AC's maximum lambda
    a0 <- a + 1; b0 <- b + 1
