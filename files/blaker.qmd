...
get_mtp <- function(x, lambda) {
  if (x == 0) {
    return(dpois(0, lambda)) # we know upper tail prob for x=0 is always 1.
  } else {
    return(min(ppois(x, lambda), ppois(x-1, lambda, lower.tail = FALSE))) # general case
  }
}
```
...
blaker_CI <- function(x, conf.level = 0.95, digits = 3) {
  ...
  p_lambda <- c() # will be used to store all *plausible* lambda's for obs x.
  
  
  for (i in 1:length(lambda)) {
    AS <- c() # initialize acceptance set vec for lambda
    # calculate obs mtp for current lambda
      ...
      for (j in 1:length(x0)) {
        ...
        if (temp_mtp <= obs_mtp) {
          AS <- c(AS, x0[j]) # AS will contain all x's resulting in MTP <= obs_mtp
        }
      }
