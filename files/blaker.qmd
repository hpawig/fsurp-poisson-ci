...
get_mtp <- function(x, lambda) {
  if (x == 0) {
    return(min(dpois(0, lambda), 1)) # we know upper tail prob for x=0 is always 1.
  } else {
    return(min(ppois(x, lambda), 1-ppois(x-1, lambda))) # general case
  }
}
```
  ...
  for (i in 1:length(lambda)) {
    ...
    # calculate obs mtp for current lambda
      ...
      for (j in 1:length(x0)) {
        ...
        } else if ((get_mtp(x0[j+1], lambda[i]) > obs_mtp) && (get_mtp(x0[j+2], lambda[i]) > obs_mtp)) {
          break
        }
      }
