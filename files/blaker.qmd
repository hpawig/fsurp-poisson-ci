  ...
  for (i in 1:length(lambda)) {
    ...
    # calculate obs mtp for current lambda
      ...
      for (j in 1:length(x0)) {
        ...
        # x's in acceptance sets MUST be consecutive. also x itself is obviously in its own CI?
        # so stop calculating mtp for x0after first x0>x that has greater mtp
        } else if ((x0[j+1] > x) && (get_mtp(x = x0[j+1], lambda[i]) > obs_mtp)) {  
          # logic: if some x0 (that is larger than observed x) 
          # has mtp above observed mtp then stop checking x's?
          break
        }
      }
...
```{r}
for (x in 0:15) {
  print(blaker_CI(x, digits = 2))
}

# stops working at x=8
