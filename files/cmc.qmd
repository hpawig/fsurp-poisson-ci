...
CI = CMC(K = 65, conf.level = 0.95, all = T); CI
# CI = CMC(K = 8, conf.level = 0.90, all = T); CI
...
CMC_CP_table <- function(max_lambda, conf.level = 0.95) {
  ...
  CI_temp <- CMC(K = 65, conf.level, all = T)
...
coverage_table_CMC.95 <- CMC_CP_table(max_lambda = 65, conf.level = 0.95)
...
```{r}
test.table <- data.frame(x = 0:150,
                         lower.CMC  = CMC(150,0.95,T)$lower,
                         upper.CMC = CMC(150,0.95,T)$upper,
                         lower.CG = CG(150,0.95,T)$lower,
                         upper.CG = CG(150,0.95,T)$upper); test.table

test.table <- test.table |> 
  mutate(lower.match = ifelse(lower.CMC == lower.CG, T, F),
         upper.match = ifelse(upper.CMC == upper.CG, T, F))

sum(test.table$lower.match); sum(test.table$upper.match); test.table
```
